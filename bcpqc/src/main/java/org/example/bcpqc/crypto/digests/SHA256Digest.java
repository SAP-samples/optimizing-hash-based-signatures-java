package org.example.bcpqc.crypto.digests;
...
//        X[xOff] = Pack.bigEndianToInt(in, inOff);
    ...
    public void processMultipleWords(byte[] in) {
        if(in.length % 4 != 0){
            throw new IllegalArgumentException("Input length no multiple of 4");
        }
    ...
    public void processZeroBytes(int zeroBytes) {
        if(zeroBytes <= 0){
            return;
        }
        if(zeroBytes % 4 != 0){
            throw new IllegalArgumentException("Byte count no multiple 0f 4");
        }
        int zeroWords = zeroBytes / 4;
        if(xOff + zeroWords >= 16) {
            throw new IllegalArgumentException("Not within block");
        }
        xOff += zeroWords;
        super.byteCount += zeroBytes;
    ...
    public void extractDigest(byte[] out, int outOff) {
