...
public class WOTSPlusParameters {
    ...
    public WOTSPlusParameters(ASN1ObjectIdentifier treeDigest, int digestSize) {
        this(treeDigest, digestSize, 16);

        String digestName = DigestUtil.getDigestName(treeDigest);
        if (oid == null) {
            throw new IllegalArgumentException("cannot find OID for digest algorithm: " + digestName);
        }
    }

    protected WOTSPlusParameters(ASN1ObjectIdentifier treeDigest, int digestSize, int winternitzParameter) {
        super();
        if (treeDigest == null) {
        ...
        logW = XMSSUtil.log2(winternitzParameter);
        ...
        len = len1 + len2;
        String digestName = DigestUtil.getDigestName(treeDigest);
        oid = WOTSPlusOid.lookup(digestName, digestSize, winternitzParameter, len);
    }
