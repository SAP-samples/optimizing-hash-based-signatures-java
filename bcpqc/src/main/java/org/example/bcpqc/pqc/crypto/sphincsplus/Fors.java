...
class Fors {
    ...
    byte[] treehash(SPHINCSPlusEngine engine, byte[] skSeed, int s, int z, byte[] pkSeed, ADRS adrsParam) {
        ...
        for (int idx = 0; idx < (1 << z); idx++) {
            ...
            while (!stack.isEmpty() && ((NodeEntry) stack.get(0)).nodeHeight == adrs.getTreeHeight()) {
    ...
    public SIG_FORS[] sign(byte[] md, byte[] skSeed, byte[] pkSeed, ADRS paramAdrs) {
        ...
        IntConsumer fn = i -> {
            ...
            SPHINCSPlusEngine engine = this.engine.clone();
            ...
            for (int j = 0; j < engine.A; j++) {
                int s = (idx / (1 << j)) ^ 1;
                authPath[j] = treehash(engine, skSeed, i * t + s * (1 << j), j, pkSeed, adrs);
            }
            sig_fors[i] = new SIG_FORS(sk, authPath);
        };

        if (HashingProviderProvider.EXECUTE_PARALLEL) {
            IntStream.range(0, engine.K)
                    .parallel()
                    .forEach(fn);
        } else {
