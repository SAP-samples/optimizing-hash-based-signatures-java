...
public class XMSSUtil {
    ...
    public static byte[] toBytesBigEndianWithSuffixes(long value, int sizeInByte, byte[] s1, byte[] s2) {
        byte[] out = new byte[sizeInByte + s1.length + s2.length];
        for (int i = (sizeInByte - 1); i >= 0; i--) {
            out[i] = (byte) value;
            value >>>= 8;
        }

        System.arraycopy(s1, 0, out, sizeInByte, s1.length);
        System.arraycopy(s2, 0, out, sizeInByte + s1.length, s2.length);
     ...
     * @param digest Digest.
     * @return Digest size.
     * @deprecated Determining digest size by digest type does not work anymore after integrating NIST parameter sets
     */
    @Deprecated
    ...
    private static class CheckingStream
        ...
        static {
            ...
            components.add("org.example.bcpqc.pqc.crypto.xmss.BDS");
            components.add("java.util.ArrayList");
            components.add("org.example.bcpqc.pqc.crypto.xmss.XMSSNode");
            components.add("[B");
            ...
            components.add("org.example.bcpqc.pqc.crypto.xmss.BDSTreeHash");
        }
