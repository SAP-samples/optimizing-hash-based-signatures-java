...
class HT {
    ...
    byte[] xmss_PKgen(byte[] skSeed, byte[] pkSeed, ADRS adrs) {
        return treehash(0, engine.H_PRIME, pkSeed, adrs);
    }
    ...
    SIG_XMSS xmss_sign(byte[] M, byte[] skSeed, int idx, byte[] pkSeed, ADRS paramAdrs) {
        ...
        for (int j = 0; j < engine.H_PRIME; j++) {
            int k = (idx / (1 << j)) ^ 1;
            AUTH[j] = treehash(k * (1 << j), j, pkSeed, adrs);
        }
    ...
    byte[] treehash(int s, int z, byte[] pkSeed, ADRS adrsParam) {
        if (HashingProviderProvider.EXECUTE_PARALLEL) {
            ThreadLocal<WotsPlus> threadLocal = ThreadLocal.withInitial(() -> wots.clone());
            ParallelTreehash parallelTreehash = new ParallelTreehash(s, z, threadLocal, new ADRS(adrsParam));
            return pool.invoke(parallelTreehash);
        }
        return treehashSequential(s, z, pkSeed, adrsParam);
    }
    ...
    byte[] treehashSequential(int s, int z, byte[] pkSeed, ADRS adrsParam) {
...
//    # Output: Boolean
    ...
    class ParallelTreehash extends RecursiveTask<byte[]> {
        private final ThreadLocal<WotsPlus> wotsPlusThreadLocal;
        ADRS adrs;
        private final int startIndex;
        private final int height;

        public ParallelTreehash(int startIndex, int height, ThreadLocal<WotsPlus> wotsPlusThreadLocal, ADRS adrs) {
            this.startIndex = startIndex;
            this.height = height;
            this.wotsPlusThreadLocal = wotsPlusThreadLocal;
            this.adrs = adrs;
        }

        @Override
        protected byte[] compute() {
            // TODO Optimize for larger tree sizes!
            if (height == 0) {
                adrs.setType(ADRS.WOTS_HASH);
                adrs.setKeyPairAddress(startIndex);

                return this.wotsPlusThreadLocal.get().pkGen(skSeed, pkSeed, adrs);
            }

            ParallelTreehash left = new ParallelTreehash(startIndex, height - 1, this.wotsPlusThreadLocal, adrs);
            ParallelTreehash right = new ParallelTreehash(startIndex + (1 << (height - 1)), height - 1, this.wotsPlusThreadLocal, new ADRS(adrs));

            right.fork();

            byte[] leftResult = left.compute();
            byte[] rightResult = right.join();

            adrs.setType(ADRS.TREE);
            adrs.setTreeHeight(height);
            adrs.setTreeIndex(startIndex / (1 << height));

            return this.wotsPlusThreadLocal.get().getEngine().H(pkSeed, adrs, leftResult, rightResult);
        }
    }
}
