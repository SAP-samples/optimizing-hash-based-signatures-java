...
public class XMSSTest
    ...
    // Fails because package has been renamed
    /*
    public void test160PrivateKeyRecovery()
        throws Exception
    {
        KeyFactory kFact = KeyFactory.getInstance("XMSS", "SAPBCPQC");
    ...
    */
    ...
    // Fails due to updated key format
    /*
    ...
    public void testXMSSSha256Signature()
            throws Exception {
        testSignature(XMSSParameterSpec.SHA256, "SHA256withXMSS");
    }

    public void testXMSSSha512Signature()
            throws Exception {
        testSignature(XMSSParameterSpec.SHA512, "SHA512withXMSS");
    }

    public void testXMSSShake128Signature()
            throws Exception {
        testSignature(XMSSParameterSpec.SHAKE128, "SHAKE128withXMSS");
    }

    public void testXMSSShake256Signature()
            throws Exception {
        testSignature(XMSSParameterSpec.SHAKE256, "SHAKE256withXMSS");
    }

    public void testXMSSSha2_192Signature()
            throws Exception {
        testSignature(XMSSParameterSpec.SHA2_192, "XMSS-SHA2_192");
    }

    public void testXMSSShake256_256Signature() throws Exception {
        testSignature(XMSSParameterSpec.SHAKE256_256, "XMSS-SHAKE256_256");
    }

    public void testXMSSShake256_192Signature() throws Exception {
        testSignature(XMSSParameterSpec.SHAKE256_192, "XMSS-SHAKE256_192");
    }


    private void testSignature(String digest, String algorithm) throws Exception {
        KeyPairGenerator kpg = KeyPairGenerator.getInstance("XMSS", "SAPBCPQC");

        kpg.initialize(new XMSSParameterSpec(10, digest), new SecureRandom());

        KeyPair kp = kpg.generateKeyPair();

        Signature xmssSig = Signature.getInstance(algorithm, "SAPBCPQC");
    ...
    public void testXMSSSha256SignatureMultiplePreHash()
        ...
        Signature sig1 = Signature.getInstance("SHA256withXMSS", "SAPBCPQC");

        Signature sig2 = Signature.getInstance("SHA256withXMSS", "SAPBCPQC");

        Signature sig3 = Signature.getInstance("SHA256withXMSS", "SAPBCPQC");
    ...
    public void testXMSSSha256KeyFactory()
            throws Exception {
        testXMSSKeyFactory(XMSSParameterSpec.SHA2_10_256, XMSSParameterSpec.SHA256);
    }

    public void testXMSSSha512KeyFactory()
            throws Exception {
        testXMSSKeyFactory(new XMSSParameterSpec(10, XMSSParameterSpec.SHA512), XMSSParameterSpec.SHA512);
    }

    public void testXMSSShake128KeyFactory()
            throws Exception {
        testXMSSKeyFactory(new XMSSParameterSpec(10, XMSSParameterSpec.SHAKE128), XMSSParameterSpec.SHAKE128);
    }

    public void testXMSSShake256KeyFactory()
            throws Exception {
        testXMSSKeyFactory(new XMSSParameterSpec(10, XMSSParameterSpec.SHAKE256), XMSSParameterSpec.SHAKE256);
    }

    public void testXMSSSha2_192KeyFactory() throws Exception {
        testXMSSKeyFactory(new XMSSParameterSpec(10, XMSSParameterSpec.SHA2_192), XMSSParameterSpec.SHA256);
    }

    private void testXMSSKeyFactory(XMSSParameterSpec parameterSpec, String expectedTreeDigest)
            throws Exception {
        KeyPairGenerator kpg = KeyPairGenerator.getInstance("XMSS", "SAPBCPQC");
        ...
        KeyFactory keyFactory = KeyFactory.getInstance("XMSS", "SAPBCPQC");

        byte[] priv = kp.getPrivate().getEncoded();
        byte[] pub = kp.getPublic().getEncoded();

        XMSSKey privKey = (XMSSKey) keyFactory.generatePrivate(new PKCS8EncodedKeySpec(priv));

        assertEquals(kp.getPrivate(), privKey);

        XMSSKey pubKey = (XMSSKey) keyFactory.generatePublic(new X509EncodedKeySpec(pub));

        assertEquals(kp.getPublic(), pubKey);

        assertEquals(parameterSpec.getHeight(), privKey.getHeight());
        assertEquals(expectedTreeDigest, privKey.getTreeDigest());

        assertEquals(parameterSpec.getHeight(), pubKey.getHeight());
        assertEquals(expectedTreeDigest, pubKey.getTreeDigest());
    }
    ...
    private void testSig(String algorithm, PublicKey pubKey, PrivateKey privKey)
        ...
        Signature s = Signature.getInstance(algorithm, "SAPBCPQC");
    ...
    public void testKeyExtraction()
        ...
        Signature sig = Signature.getInstance("SHA256withXMSS", "SAPBCPQC");
    ...
    public void testExhaustion()
            throws Exception {
        StateAwareSignature s1 = (StateAwareSignature) Signature.getInstance(BCObjectIdentifiers.xmss_SHA256.getId(), "SAPBCPQC");
        Signature s2 = Signature.getInstance(BCObjectIdentifiers.xmss_SHA256.getId(), "SAPBCPQC");
    ...
    public void testShardedKeyExhaustion()
            throws Exception {
        Signature s1 = Signature.getInstance(BCObjectIdentifiers.xmss_SHA256.getId(), "SAPBCPQC");
        Signature s2 = Signature.getInstance(BCObjectIdentifiers.xmss_SHA256.getId(), "SAPBCPQC");
    ...
    private void exhaustKey(
        ...
        KeyFactory keyFact = KeyFactory.getInstance("XMSS", "SAPBCPQC");
    ...
    public void testNoRepeats()
        ...
        Signature sigGen = Signature.getInstance(BCObjectIdentifiers.xmss_SHA256.getId(), "SAPBCPQC");
        Signature sigVer = Signature.getInstance(BCObjectIdentifiers.xmss_SHA256.getId(), "SAPBCPQC");
    ...
    private void testPrehashAndWithoutPrehash(String baseAlgorithm, String digestName, Digest digest)
            throws Exception {
        Signature s1 = Signature.getInstance(digestName + "with" + baseAlgorithm, "SAPBCPQC");
        Signature s2 = Signature.getInstance(baseAlgorithm, "SAPBCPQC");
    ...
    private void testPrehashAndWithoutPrehash(ASN1ObjectIdentifier oid1, ASN1ObjectIdentifier oid2, String digestName, Digest digest)
            throws Exception {
        Signature s1 = Signature.getInstance(oid1.getId(), "SAPBCPQC");
        Signature s2 = Signature.getInstance(oid2.getId(), "SAPBCPQC");
    ...
    public void testReserialization()
        ...
        for (int i = 0; i != 10; i++) {
            Signature signer = Signature.getInstance(sigAlg, "SAPBCPQC");
            ...
            Signature verifier = Signature.getInstance(sigAlg, "SAPBCPQC");
