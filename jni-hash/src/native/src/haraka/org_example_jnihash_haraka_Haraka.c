/*
 * haraka optimized reference implementation.
 * Author: Stefan Kölbl
 * Edited for JNI compability by Lena Heimberger
 */
#include "../../include/org_example_jnihash_haraka_Haraka.h"
#include "haraka.h"
#include <string.h>
#include <stdio.h>

JNIEXPORT void JNICALL Java_org_example_jnihash_haraka_Haraka_load_1constants(JNIEnv *env, jobject obj)
{
...
JNIEXPORT void JNICALL Java_org_example_jnihash_haraka_Haraka_set_1constants(JNIEnv *env, jobject obj, jintArray arr)
{
    jint *in = (*env)->GetIntArrayElements(env, arr, 0);
    if ((*env)->GetArrayLength(env, arr) != 160)
    {
        printf("Problem with round constants: there are %d\n", (*env)->GetArrayLength(env, arr));
        return;
    }
    for (int i = 0; i < 40; i++)
        rc[i] = _mm_set_epi32(in[i * 4 + 3], in[(i * 4) + 2], in[(i * 4) + 1], in[(i * 4)]);
    (*env)->ReleaseIntArrayElements(env, arr, in, 0);
}

JNIEXPORT void JNICALL Java_org_example_jnihash_haraka_Haraka_set_1secret_1constants(JNIEnv *env, jobject obj, jintArray arr)
{
    jint *in = (*env)->GetIntArrayElements(env, arr, 0);
    if ((*env)->GetArrayLength(env, arr) != 160)
    {
    ...
    for (int i = 0; i < 40; i++)
        sseed_rc[i] = _mm_set_epi32(in[i * 4 + 3], in[(i * 4) + 2], in[(i * 4) + 1], in[(i * 4)]);
    (*env)->ReleaseIntArrayElements(env, arr, in, 0);
}
 ...
 * Signature: ([B[B)V
 */
JNIEXPORT void JNICALL Java_org_example_jnihash_haraka_Haraka_secret_1haraka256(JNIEnv *env, jobject obj, jbyteArray outarr, jbyteArray inarr)
{
    jboolean *copy = JNI_FALSE;
    jbyte *in = (*env)->GetByteArrayElements(env, inarr, 0);
    jbyte *out = (*env)->GetByteArrayElements(env, outarr, copy);
    __m128i s[2], tmp;
...
JNIEXPORT void JNICALL Java_org_example_jnihash_haraka_Haraka_haraka256(JNIEnv *env, jobject obj, jlong bufAddress)
{
    ...
    s[1] = _mm_aesenc_si128(s[1], rc[0 + 1]);
    s[0] = _mm_aesenc_si128(s[0], rc[0 + 2]);
    s[1] = _mm_aesenc_si128(s[1], rc[0 + 3]);
    MIX2(s[0], s[1]);
    ...
    s[0] = _mm_xor_si128(s[0], LOAD(buf));
    s[1] = _mm_xor_si128(s[1], LOAD(buf + 16));

    STORE(buf, s[0]);
    STORE(buf + 16, s[1]);
}

JNIEXPORT void JNICALL Java_org_example_jnihash_haraka_Haraka_haraka512perm(JNIEnv *env, jobject obj, jlong bufAddress)
{
    char *buf = (char *)bufAddress;
    u128 s[4], tmp;
...
JNIEXPORT void JNICALL Java_org_example_jnihash_haraka_Haraka_haraka512(JNIEnv *env, jobject obj, jlong bufAddress)
{
    char *buf = (char *)bufAddress;
    u128 s[4], tmp;
    ...
    s[0] = _mm_xor_si128(s[0], LOAD(buf));
    s[1] = _mm_xor_si128(s[1], LOAD(buf + 16));
    s[2] = _mm_xor_si128(s[2], LOAD(buf + 32));
    s[3] = _mm_xor_si128(s[3], LOAD(buf + 48));

    TRUNCSTORE(buf, s[0], s[1], s[2], s[3]);
}
...
JNIEXPORT jint JNICALL Java_org_example_jnihash_haraka_Haraka_check_1for_1native_1instructions(JNIEnv *env, jclass klass)
{
#if __APPLE__
    // apple specific code
#elif _WIN32
